...
- (void)initViews {
    ...
    self.contentRootView = [[UIView alloc] initWithFrame:CGRectMake(UMComPostCellContentOffsetX, 0, self.frame.size.width, 100)];
    ...
    _nickNameLabel.textColor = UMComColorWithColorValueString(FontColorForFeedNickName);
    ...
    _locationLabel.textColor = UMComColorWithColorValueString(FontColorForFeedLocation);
    
    _likeLabel.font = UMComFontNotoSansLightWithSafeSize(8);
    _likeLabel.textColor = UMComColorWithColorValueString(FontColorForFeedLike);
    _likeLabel.textAlignment = NSTextAlignmentLeft;
    
    _commentLabel.font = UMComFontNotoSansLightWithSafeSize(8);
    _commentLabel.textColor = UMComColorWithColorValueString(FontColorForFeedComment);
...
- (NSUInteger)refreshBottomWithOriginY:(CGFloat)originY {
    ...
    originX = _contentRootView.frame.size.width - 110;
    ...
    _likeLabel.frame = CGRectMake(originX, originY, 40, UMComPostCellBottomLabelHeight);
    originX += _likeLabel.frame.size.width;
    ...
    _commentLabel.frame = CGRectMake(originX, originY, 40, UMComPostCellBottomLabelHeight);
...
- (void)refreshLayout {
    ...
    if (self.cell_top_edge != 0) {
        frame.origin.y = self.cell_top_edge;
    }
    frame.size.width = self.contentView.frame.size.width - frame.origin.x * 2;
    frame.size.height = cellHeight - UMComPostCellContentOffsetY * 2;
    ...
    _likeLabel.text = [NSString stringWithFormat:@"%@", countString(_postFeed.likes_count)];
    _commentLabel.text = [NSString stringWithFormat:@"%@", countString(_postFeed.comments_count)];

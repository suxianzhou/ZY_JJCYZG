...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        self.likeCountLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.f, 0.f, 40.f, UMComPostIconWidth)];
...
- (void)refreshImageLayout
{
    ...
    if (self.imageUrls.count > 0) {
        [_imagePreviewView removeFromSuperview];
        self.imagePreviewView = [[UMComGridView alloc] init];
...
- (void)refreshFooterLayout
{
    if (self.comment.reply_comment.commentID) {
        ...
        // Header bar
        NSString *repliedHeaderString = [NSString stringWithFormat:@"%@ 于 %@ 发表在 %ld楼", repliedComment.creator.name, createTimeString(repliedComment.create_time), [repliedComment.floor integerValue]];
        ...
        if ([repliedComment.status integerValue] > 1) {
            ...
            if (repliedComment.image_urls.array.count > 0) {
                summaryText = [summaryText stringByAppendingString:@" [图片]"];
            }
    ...
    _dateLabel.text = createTimeString(self.comment.create_time);
...
- (void)updateActionButtonStatus
{
    ...
    _likeCountLabel.text = [NSString stringWithFormat:@"%@", countString(self.comment.likes_count)];
}

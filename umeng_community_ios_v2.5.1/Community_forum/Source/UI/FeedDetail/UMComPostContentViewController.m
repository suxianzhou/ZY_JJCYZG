...
static NSString *UMComPostHtmlContentCellIdentifier = @"UMComPostHtmlContentCellIdentifier";

- (id)initWithFeed:(NSString *)feedId
         viewExtra:(NSDictionary *)viewExtra;
{
    self.feedID = feedId;
    self.wakedCommentID = [viewExtra valueForKey:@"comment_id"];
...
- (void)viewDidLoad {
    ...
    [self.tableView registerClass:[UMComPostHtmlContentBodyCell class] forCellReuseIdentifier:UMComPostHtmlContentCellIdentifier];
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (self.bottomBarView.superview != self.navigationController.view) {
        [self.view.window addSubview:_bottomBarView];
    }
...
- (void)watchHost:(id)sender
{
    ...
    _watchHostButton.userInteractionEnabled = NO;
...
- (void)displayPostMenu:(id)sender
{
    [UMComLoginManager performLogin:self completion:^(id responseObject, NSError *error) {
        if (!error) {
            [self displayPostMenuForLogedIn];
        }
    }];
}

- (void)displayPostMenuForLogedIn
{
    if (_menuList) {
    ...
    if ([self checkAuthDeletePost]) {
        [_menuList addObject:@{UMComFeedMenuName: UMComLocalizedString(@"um_com_delete", @"删除"),
    ...
    [_menuList addObject:@{UMComFeedMenuName: UMComLocalizedString(@"um_com_replyComment", @"回复"),
                           UMComFeedMenuSelector: NSStringFromSelector(@selector(replyPost))}];
    /*
     //暂时屏蔽禁言的显示，保留其功能 version 2.5
    if ([_feed.ban_user integerValue] == 1) {
        NSString *tip = UMComLocalizedString(@"um_com_forbidPosted", @"禁言");
...
//        }
     ...
     */
    [_menuList addObject:@{UMComFeedMenuName: UMComLocalizedString(@"um_com_share", @"分享"),
                           UMComFeedMenuSelector: NSStringFromSelector(@selector(sharePost))}];
    
    if ([self checkNeedReport]) {
        [_menuList addObject:@{UMComFeedMenuName: UMComLocalizedString(@"um_com_spamContent", @"举报内容"),
                               UMComFeedMenuSelector: NSStringFromSelector(@selector(reportPost))}];

    }
    
    if ([self checkNeedReportCreatorWithFeed:self.feed]) {
        [_menuList addObject:@{UMComFeedMenuName: UMComLocalizedString(@"um_com_spamUser", @"举报用户"),
                                              UMComFeedMenuSelector: NSStringFromSelector(@selector(reportUser))}];
    }

    [_menuList addObject:@{UMComFeedMenuName: UMComLocalizedString(@"um_com_copy", @"拷贝"),
...
- (void)displayCommentMenu
{
    [UMComLoginManager performLogin:self completion:^(id responseObject, NSError *error) {
        if (!error) {
            [self displayCommentMenuForLogedIn];
        }
    }];
}

- (void)displayCommentMenuForLogedIn
{
    if (_CommentMenuList) {
    ...
    if ([self checkAuthDeleteComment]) {
        [_CommentMenuList addObject:@{UMComFeedMenuName: UMComLocalizedString(@"um_com_delete", @"删除"),
    ...
    [_CommentMenuList addObject:@{UMComFeedMenuName: UMComLocalizedString(@"um_com_replyComment", @"回复"),
...
//        }
    ...
    if ([self checkNeedReportForCurComment:self.currentOpComment]) {
        [_CommentMenuList addObject:@{UMComFeedMenuName: UMComLocalizedString(@"um_com_spamContent", @"举报内容"),
...
- (void)createBottomBar
{
    ...
    UIView *bottomBar = [[UIView alloc] initWithFrame:CGRectMake(0,
                                                                 self.view.window.bounds.size.height - barHeight,
...
//    }
    UMComOneFeedRequest *req = [[UMComOneFeedRequest alloc] initWithFeedId:_feedID viewExtra:self.viewExtra];
    [req fetchRequestFromServer:^(NSArray *data, BOOL haveNextPage, NSError *error) {
        if (error) {
            ...
            if ([data[0] isKindOfClass:[UMComFeed class]]) {
                ...
                NSString *feedKey = [NSString stringWithFormat:@"%@_%@", UMComFeedIdentifierPrefix, feed.feedID];
                [ws.cacheHeightInfo removeObjectForKey:feedKey];
...
- (void)replyComment
{
    [UMComLoginManager performLogin:self completion:^(id responseObject, NSError *error) {
        if (!error) {
            ...
            tempReplyController.commentcreator =  self.currentOpComment.creator.name;
            tempReplyController.commitBlock = ^(NSString *content, NSArray *imageList) {
                [weakself replyContent:content toPost:_feed fromComment:_currentOpComment imageList:imageList];
                NSLog(@"replyComment commit");
                [weakself dismissViewControllerAnimated:YES completion:nil];
            };
            tempReplyController.cancelBlock = ^{
                NSLog(@"replyComment canncel");
...
- (void)replyPost
{
    ...
    [UMComLoginManager performLogin:self completion:^(id responseObject, NSError *error) {
        if (!error) {
            ...
            UMComReplyEditViewController* tempReplyController =  [[UMComReplyEditViewController alloc] init];
            tempReplyController.commitBlock = ^(NSString *content, NSArray *imageList) {
                [weakself replyContent:content toPost:weakself.feed fromComment:nil imageList:imageList];
                NSLog(@"replyPost commit");
                [weakself dismissViewControllerAnimated:YES completion:nil];
            };
            tempReplyController.cancelBlock = ^{
                NSLog(@"replyPost canncel");
    ...
    if ([self.feed.ban_user integerValue] == 1) {
        [UMComLoginManager performLogin:self completion:^(id responseObject, NSError *error) {
            if (!error) {
                ...
                [UMComPushRequest banUser:self.feed.creator inTopics:self.feed.topics.array ban:YES completion:^(id responseObject, NSError *error) {
                    if (error) {
                        [UMComShowToast showFetchResultTipWithError:error];
                    } else {
                        NSString *tip = @"禁言成功";
    ...
    if ([_currentOpComment.ban_user integerValue] == 1) {
        [UMComLoginManager performLogin:self completion:^(id responseObject, NSError *error) {
            if (!error) {
                ...
                [UMComPushRequest banUser:_currentOpComment.creator inTopics:self.feed.topics.array ban:YES completion:^(id responseObject, NSError *error) {
                    if (error) {
                        [UMComShowToast showFetchResultTipWithError:error];
                    } else {
                        NSString *tip = @"禁言成功";
...
- (void)reportUser
{
    [UMComLoginManager performLogin:self completion:^(id responseObject, NSError *error) {
        if (!error) {
            [UMComPushRequest spamWithUser:self.feed.creator completion:^(NSError *error) {
                if (error) {
                    [UMComShowToast showFetchResultTipWithError:error];
                } else {
                    [[UMComiToast makeText:@"举报成功"] show];
...
- (void)switchToTopic:(UMComTopic *)topic
{
...
- (void)switchURLWithLinkRequest:(NSURLRequest *)request
{
    NSString* hrefString =  request.URL.absoluteString;
    UMComWebViewController * webViewController = [[UMComWebViewController alloc] initWithUrl:hrefString];
    [self.navigationController pushViewController:webViewController animated:YES];
}
...
//    if (![commentCell.comment.commentID isEqualToString:comment.commentID]) {
        ...
        if (!commentText) {
            ...
            commentText = [UMComMutiText mutiTextWithSize:CGSizeMake(self.view.frame.size.width - UMComPostContentAvatarSize - UMComPostPad - UMComPostOriginX * 2, INT_MAX)
                                                ...
                                                lineSpace:2.f
                                               checkWords:nil
                                                textColor:UMComColorWithColorValueString(UMComPostColorGray)];
...
- (UMComPostHtmlContentBodyCell *)createPostHtmlContentBodyCell
{
    if (!_cachedPostBodyCell) {
        UMComPostHtmlContentBodyCell *cell = [self.tableView dequeueReusableCellWithIdentifier:UMComPostHtmlContentCellIdentifier];
    ...
    [_cachedPostBodyCell refreshLayoutWithCalculatedTextObj:nil andFeed:_feed];
    
    return _cachedPostBodyCell;
}

- (NSUInteger)heightForInitializedPostHtmlContentCell
{
    NSUInteger height = 0;
    NSString *feedKey = [NSString stringWithFormat:@"%@_%@", UMComFeedIdentifierPrefix, _feed.feedID];
    if ([_cacheHeightInfo objectForKey:feedKey]) {
        ...
        UMComPostHtmlContentBodyCell *cell = [self createPostHtmlContentBodyCell];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    if (section == 0) {
        ...
        if (!self.haveNextPage && count < BatchSize) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        if ([self isHtmlData]) {
            NSUInteger height = [self heightForInitializedPostHtmlContentCell];
            return height;
        }
        else{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        if ([self isHtmlData]) {
            retCell = [self createPostHtmlContentBodyCell];
            UMComPostHtmlContentBodyCell* htmlCell =  (UMComPostHtmlContentBodyCell*)retCell;
            htmlCell.indexPath = indexPath;
        }
        else{
...
{
    _watchHostButton.userInteractionEnabled = YES;
    if (!error && [data isKindOfClass:[NSArray class]]) {
        self.dataArray = data;
        [_sentCommentList removeAllObjects];
        self.displayedCommentList = self.dataArray;
...
#pragma mark - overide from UIViewController

-(void)viewWillLayoutSubviews
{
    [super viewWillLayoutSubviews];
    
//    if (UMComSystem_Version_Less_Than(@"7.0"))
//    {
//        if (!_bottomBarView) {
//            [self createBottomBar];
//        }
//        
//        NSUInteger barHeight = UMComPostBottomBarHeight;
//        _bottomBarView.frame =    CGRectMake(0,
//                                             self.navigationController.view.frame.size.height - barHeight,
...
{
    NSString *feedKey = [NSString stringWithFormat:@"%@_%@", UMComFeedIdentifierPrefix, self.feed.feedID];
    [self.cacheHeightInfo removeObjectForKey:feedKey];
    
    [self.cacheHeightInfo setObject:[NSNumber numberWithInteger:cell.totalHeight] forKey:feedKey];
    
    [self.tableView beginUpdates];
    NSArray* reloadArray = @[cell.indexPath];
    [self.tableView reloadRowsAtIndexPaths:reloadArray withRowAnimation:NO];
...
-(void)handleClickCell:(UMComPostHtmlContentBodyCell*)cell withDataObject:(id)object userInfo:(NSDictionary*)userInfo
{
    if (object) {
        ...
        if ([object isKindOfClass:[UMComUser class]])
        {
            [self switchToUser:object];
        }
        else if([object isKindOfClass:[UMComTopic class]])
        {
            [self switchToTopic:object];
        }
        else if ([object isKindOfClass:[NSURLRequest class]])
        {
            [self switchURLWithLinkRequest:object];
        }
        else{}
    }
}

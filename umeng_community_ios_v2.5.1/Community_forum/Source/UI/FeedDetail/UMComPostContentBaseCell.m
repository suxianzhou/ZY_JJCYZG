...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(clikOnCell)];
...
- (CGFloat)refreshUserInfoBar
{
    ...
    if (_isComment) {
        ...
        frame.origin.y += UMComPostPad + UMComPostPad / 2.f;
    }
    ...
    if (self.user.medal_list.array.count > 0) {
        NSUInteger displayCount = 1;
        // fill array
        NSInteger allocCount = displayCount - _medalViews.count;
    ...
    if (_isComment) {
        ...
        userInfoHeight = _nickLabel.frame.size.height + UMComPostPad;
    } else {
        _cellHeight += _avatar.frame.size.height + UMComPostPad;
        userInfoHeight = _avatar.frame.size.height + UMComPostPad;
    }
...
- (CGFloat)refreshBodyWithMutiText:(UMComMutiText *)textObj
{
    ...
    bodyHeight = _bodyTextView.frame.size.height + UMComPostPad * 1.5;
    return bodyHeight;
}
...
{
    if (self.actionBlock) {
        self.actionBlock(self, UMComPostContentActionMenu);
    }
}
@end
